...
const ownerReq = (address = VALID_ADDR) => ({
  params: { address },
  user: { userId: 'user-1' },
} as any);
...
test('getStatus returns not_registered for unknown address', async () => {
  ...
  await getStatus(ownerReq(), res as any);
...
test('getStatus returns approved for registered institution', async () => {
  ...
  prisma.institution.findUnique = async () => ({
    ...
    userId: 'user-1', kycStatus: 1, countryCode: 840, merkleIndex: 0,
...
test('getStatus returns pending for non-approved institution', async () => {
  ...
  prisma.institution.findUnique = async () => ({
    ...
    userId: 'user-1', kycStatus: 0, countryCode: 840, merkleIndex: null,
...
test('getStatus rejects invalid address format', async () => {
  const res = createMockResponse();
  await getStatus(ownerReq('bad'), res as any);
...
test('getStatus rejects access to another user institution', async () => {
  const res = createMockResponse();
  prisma.institution.findUnique = async () => ({
    id: 'inst1', name: 'Other', walletAddress: VALID_ADDR,
    userId: 'user-2', kycStatus: 1, countryCode: 840, merkleIndex: 0,
    approvedAt: '2026-01-01', createdAt: '2026-01-01',
  } as any);
...
test('getAttestation returns 404 for unregistered wallet', async () => {
  ...
  await getAttestation(ownerReq(), res as any);
...
test('getAttestation returns 404 for non-approved institution', async () => {
  const res = createMockResponse();
  prisma.institution.findUnique = async () => ({
    id: 'inst1', walletAddress: VALID_ADDR, userId: 'user-1', kycStatus: 0,
...
test('getAttestation returns 500 when merkle index is missing', async () => {
  const res = createMockResponse();
  prisma.institution.findUnique = async () => ({
    id: 'inst1', walletAddress: VALID_ADDR, userId: 'user-1', kycStatus: 1,
    countryCode: 840, merkleIndex: null,
  } as any);
...
test('getAttestation rejects invalid address', async () => {
  const res = createMockResponse();
  await getAttestation(ownerReq('invalid'), res as any);
  assert.equal(res.statusCode, 400);
});

test('getAttestation rejects access to another user institution', async () => {
  const res = createMockResponse();
  prisma.institution.findUnique = async () => ({
    id: 'inst1', walletAddress: VALID_ADDR, userId: 'user-2', kycStatus: 1,
    countryCode: 840, merkleIndex: 0,
  } as any);
